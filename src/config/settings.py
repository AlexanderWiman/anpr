...
class Settings(BaseSettings):
    ...
    detection_roi_top_fraction: float = Field(
        default=0.35, alias="DETECTION_ROI_TOP_FRACTION"
    )
    # Optional per-camera override JSON, e.g.:
    # {"entrance-1":{"band":"top","fraction":0.45},"entrance-2":{"band":"bottom","fraction":0.40}}
    detection_roi_by_camera: str = Field(default="", alias="DETECTION_ROI_BY_CAMERA")
